...
export default function Products() {
  ...
  // ── NEW: permissions state fetched from API ──────────────────
  ...
  // ── NEW: fetch permissions from backend on mount ─────────────
  useEffect(() => {
    const fetchPermissions = async () => {
      try {
        const res = await API.get('/auth/my-permissions');
        setPermissions(res.data);
        // res.data = { can_create: true, can_read: true,
  ...
  // ── NEW: show spinner until permissions are loaded ────────────
  if (permLoading) {
    return (
      <div className="products-page">
        <Navbar />
        <div className="products-loading" style={{ marginTop: 80 }}>
    ...
    <div className="products-page">
      ...
      <div className="products-container">
        ...
        <div className="products-topbar">
          ...
          <div className="products-actions">
            <div className="search-wrap">
              <svg className="search-icon" width="14" height="14"
            ...
            {/* canCreate now comes from DB, not hardcode */}
            {canCreate && (
          ...
          {canCreate && (
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button className="btn btn-primary btn-sm"
                onClick={() => { setEditProduct(null); setShowModal(true); }}>Save</button>
            </div>
          )}
        </div>
      </div>
